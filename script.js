...
function updateUserSessionUI() {
    const div = document.getElementById("user-session");
    if (!div) return;
    const token = localStorage.getItem("authToken"), name = localStorage.getItem("userName");
    if (token && name) {
        div.innerHTML = `<div style="display:flex;align-items:center;gap:15px;"><span style="color:#ccc;">أهلاً، ${name}</span><button onclick="logout()" class="btn" style="width:auto;padding:8px 15px;margin:0;font-size:0.9rem;"><i class="fas fa-sign-out-alt"></i></button></div>`;
    } else {
...
async function loadProducts() {
    ...
    try {
        ...
        console.error(e);
        container.innerHTML = "<p>خطأ في عرض الباقات.</p>";
    }
}
...
async function loadMyOrdersWidget() {
    ...
    const token = localStorage.getItem("authToken");
    if (!token) {
        section.innerHTML = '<p>يرجى <a href="login.html">تسجيل الدخول</a> لعرض طلباتك.</p>';
        return;
    }
    ...
    try {
        ...
        section.innerHTML = "";
        if (!orders.length) {
            section.innerHTML = "<p>لا توجد طلبات سابقة.</p>";
            return;
        }
        orders.forEach(o => {
...
async function loadAdminOrders() {
    ...
    try {
        ...
        orders.forEach(o => {
            ...
            const actionBtn = status === 'pending' ? `<button class="btn" onclick="approve(${o.orderId}, this)">تفعيل</button>` : `<span>${status === 'completed' ? "مكتمل" : "تم"}</span>`;
            container.innerHTML += `<div class="order-card" id="order-${o.orderId}"><div>${receiptHtml}<div><h4>${o.productName}</h4><p>#${o.orderId} | ${o.userPhone}</p></div></div><div>${actionBtn}</div></div>`;
        });
    } catch (e) {
        console.error(e);
        container.innerHTML = "<p>خطأ تحميل.</p>";
    }
}
