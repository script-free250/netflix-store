...
async function submitOrder(e) {
    ...
    btn.disabled = false; btn.innerText = "تأكيد وإرسال";
}
...
function loadMyOrdersWidget() {
    ...
    if (!localOrders.length) { section.innerHTML = '<p style="color:#555; text-align:center;">لا توجد طلبات.</p>'; return; }
...
async function loadAdminOrders() {
    ...
    try {
        ...
        orders.forEach(o => {
            ...
            const receiptHtml = receiptUrl ? `<a href="${receiptUrl}" target="_blank"><img src="${receiptUrl}" class="receipt-thumb" title="اضغط للتكبير"></a>` : '<span style="color:red">بدون صورة</span>';

            const isPending = o.status === 'pending';
            const actionBtn = isPending ? `<button class="btn" style="width:auto; padding:5px 10px; font-size:0.8rem; background:#46d369; color:#000;" onclick="approve(${o.orderId})">✅ تفعيل</button>` : '<span style="color:#46d369">مفعل</span>';

            container.innerHTML += `
                <div class="card" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px; padding:15px; background:#1a1a1a; flex-direction:row;">
                    <div style="display:flex; gap:15px; align-items:center;">
                        ${receiptHtml}
                        <div>
                            <div style="color:var(--primary); font-weight:bold;">${o.productName}</div>
                            <div style="font-size:0.85rem; color:#888;">#${o.orderId}</div>
                            <div style="font-size:0.9rem; color:#fff;">📞 ${o.userPhone}</div>
                        </div>
                    </div>
                    <div>${actionBtn}</div>
                </div>`;
...
async function approve(id) {
    if(!confirm("هل راجعت الإيصال؟")) return;
    try {
